...
:white_check_mark:  **Do:** Commonly the test will need to add records to columns with unique constraints. Since multiple tests ae likely to add the same value, add tiny random value as a suffix to keep the records unique. Collissions between tests are more likely to occur if the DB is not cleaning-up after each test (See bullet: Choose a clear data clean-up strategy)
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE4OTcyMDk1MDEsLTgwNDUyMzk3MywtMT
